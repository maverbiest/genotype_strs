...
df_genotypes_pt <- read.csv("/Users/maxverbiest/PhD/projects/genotype_strs/results/oxana_variation/crc_primarytumor_11may.csv") %>% 
  filter(patient %in% shared_patients)
df_genotypes_bdn <- read.csv("/Users/maxverbiest/PhD/projects/genotype_strs/results/oxana_variation/crc_bloodnormal_10may.csv") %>% 
  filter(patient %in% shared_patients)
...
# This filter takes DF from 2615301 -> 2470982 rows
df_patient_variation_filt <- df_patient_variation %>% 
  left_join(df_str_info %>% select(tmp_id, in_segdup), by="tmp_id") %>% 
  filter(!in_segdup) %>% 
  select(-in_segdup)

# This filter takes DF from 2470982 -> 1542994 rows (remove homorepeats and repeats with close neighbours)
...
# This removes 15 MSS patients that have very few called loci (84 -> 69 patients)
df_patient_variation_filt <- df_patient_variation_filt %>% 
  group_by(patient) %>% 
  filter(n() > 5000)
```
...
#   summarize(calls = n()) %>%
#   rbind(data_frame(patient=c("TCGA-AA-3525"), sample_type=c("Primary Tumor"), calls=(0))) %>% # lovely little hard coding
#   left_join(df_clinical, by="patient") %>% 
...
df_locus_variability <- df_patient_variation_filt %>%
  ...
  summarize(count=n(), avg_size_diff = mean(patient_len_diff)) %>% 
  pivot_wider(
    names_from = instable,
    values_from = count,
